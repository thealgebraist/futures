...
std::vector<float> load_column(int col_idx) {
    std::vector<float> data;
    std::ifstream file("data/energy/returns_15m.csv");
...
int main(int argc, char** argv) {
    ...
    for (int i = train_split; i < n_samples - 1; ++i) {
        ...
        float threshold = 0.001f / (round + 1); // Iterative improvement: adjust threshold
        if (p > threshold) capital += capital * actual - 0.1f;
        else if (p < -threshold) capital -= capital * actual + 0.1f;
