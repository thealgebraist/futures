...
struct VecPRNG {
    ...
    float32x4_t next_f32() {
        ...
        return vsubq_f32(vreinterpretq_f32_u32(mantissa), vdupq_n_f32(1.0f));
    }
};
    ...
    float evaluate(float x, int neuron_idx) {
        switch (type) {
            ...
            case UNIF_NEG1_1: {
                float32x4_t r = prng.next_f32();
                float rand_val;
                vst1q_lane_f32(&rand_val, r, 0); // 0 to 1
                return std::max(0.0f, x) + (rand_val * 2.0f - 1.0f) * 0.05f;
            }
            case UNIF_ZERO_1: {
                float32x4_t r = prng.next_f32();
                float rand_val;
                vst1q_lane_f32(&rand_val, r, 0);
                return std::max(0.0f, x) + rand_val * 0.05f;
            }
        }
        return 0;
    }
...
Result run_benchmark(ActType type, std::string name, std::vector<Problem>& problems) {
    ...
    std::cout << "Running 10s benchmark for: " << name << "..." << std::endl;
    for(auto& prob : problems) {
        ...
        while(true) {
            ...
            final_loss = 0.999f * final_loss + 0.001f * loss;
    ...
    return {name, total_reduction / problems.size() / 10.0f, (float)total_steps / total_time};
}
...
int main() {
    ...
    results.push_back(run_benchmark(ALIAS, "Alias (Opt)", problems));
    ...
    results.push_back(run_benchmark(UNIF_NEG1_1, "Stoch Uniform [-1,1]", problems));
    results.push_back(run_benchmark(UNIF_ZERO_1, "Stoch Uniform [0,1]", problems));
