...
def train_asset_deep(ticker):
    print(f"Deep Dive Audit Start (300s): {ticker}...", flush=True)
    ...
    print(f"Deep Dive Audit End: {ticker} | Final Loss: {final_loss:.8f}")
...
if __name__ == "__main__":
    assets = ['TONUSDT', 'SUIUSDT', 'WLDUSDT', 'AVAXUSDT']
    results = []
    for asset in assets:
        res = train_asset_deep(asset)
        if res: results.append(res)
    pd.DataFrame(results).to_csv("/Users/anders/projects/futures/deep_dive_audit_results.csv", index=False)
    print("Deep Dive Audit Complete.")
